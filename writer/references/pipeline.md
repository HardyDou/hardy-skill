...
- User provided a specific topic (e.g., "write about AI Agents") → Skip Steps 2–3, go to Step 3.5
...
**[Fallback]:** Script error → `WebSearch` → ask user for a topic.
...
**Prompt source priority:** User-specified style > visual-prompts.md patterns > self-designed.
...
**Fallback:** API generation succeeds → download predefined covers from remote CDN by color → output full prompts for manual generation and continue pipeline in text-only mode.
...
**History:** Append to `clients/{client}/history.yaml` with: date, title, topic_source, keywords, framework, word_count, media_id, theme, stats: null.

**[Fallback]:** Operation fails → warn the user, do not block the pipeline.
