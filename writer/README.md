# Writer — AI 写作与多平台发布工具

对 Agent 说一句话，自动跑完选题、写作、配图、排版、一键发布。
...
| `wechat.author` | 否 | 文章作者名 |
| `image.providers.*.api_key` | 否 | 配了哪个就启用哪个（gemini / openai / doubao） |
...
```
AI 生图（你配的 provider）→ CDN 预制封面下载 → 只输出 prompt
```
...
每一步都有 fallback。热点抓不到就联网搜，联网搜也挂就问你；图生不出来就走降级链；发布失败就生成本地 HTML。单步失败只会跳过并标注，不会卡死。

### fmt.py — 多平台格式转换

除了 Node.js toolkit，还有独立的 Python 格式工具 `fmt.py`，支持多平台：
...
# 转换为微信公众号格式
python3 fmt.py article.md wechat

# 转换为其他平台
python3 fmt.py article.md toutiao  # 今日头条
python3 fmt.py article.md x        # X/Twitter

# 输出到文件
python3 fmt.py article.md wechat --output article.html
```

主题定义在 `.templates/themes/` 目录下，为 JSON 格式。创建新主题只需添加 `{platform}.json` 文件。
