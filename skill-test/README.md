...
- 💰 **成本追踪**：记录 token 使用量和 API 成本
- 📊 **质量评分**：从输出质量、响应速度、指令遵循、实用性、成本效率五个维度评分
- 📄 **报告生成**：自动生成单个 skill 的 REPORT.md 和汇总 SUMMARY.md
- 📤 **过程导出**：导出完整对话过程到 process.md
...
├─ 保存产出物到 output.*
├─ 运行 /usage 获取 token 使用量
├─ 运行 /cost 获取成本信息
├─ 运行 /export 导出对话到 process.md
├─ 生成 REPORT.md（评分 + 成本分析 + 亮点 + 不足）
└─ 输出测评摘要
...
| **实用性** | 对目标用户的实际价值 |
| **成本效率** | Token 使用量和 API 成本相对于产出价值的合理性 |

**总分**：25 分

### 成本效率评分标准

- **5 分**：极低成本，高价值（< 5k tokens）
- **4 分**：合理成本（5k-15k tokens）
- **3 分**：中等成本（15k-30k tokens）
- **2 分**：较高成本（30k-50k tokens）
- **1 分**：过高成本（> 50k tokens）
